		...
		Context("linux", func() {
			BeforeEach(func() {
				if !useExistingApp {
					...
					Expect(cf.Cf("push", appName, "-b", testConfig.BinaryBuildpack, "-m", "16M", "-k", "16M", "-p", smoke.SimpleBinaryAppBitsPath, "-d", testConfig.AppsDomain).Wait(testConfig.GetPushTimeout())).To(Exit(0))
				} else {
